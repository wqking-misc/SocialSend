...
UniValue obfuscation(const UniValue& params, bool fHelp)
{
    ...
    SendMoney(address, nAmount, wtx, ONLY_DENOMINATED);
    //    if (strError != "")
    //        throw JSONRPCError(RPC_WALLET_ERROR, strError);
...
UniValue getpoolinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "\nResult:\n"
            "{\n"
            "  \"current\": \"addr\",    (string) send address of current masternode\n"
...
UniValue masternode(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp ||
        ...
        throw runtime_error(
            ...
            "  start        - Start masternode configured in send.conf\n"
...
UniValue listmasternodes(const UniValue& params, bool fHelp)
{
    ...
    BOOST_FOREACH (PAIRTYPE(int, CMasternode) & s, vMasternodeRanks) {
        ...
        if (mn != NULL) {
            ...
            obj.push_back(Pair("addr", EncodeDestination(CTxDestination(mn->pubKeyCollateralAddress.GetID()))));
...
UniValue masternodecurrent (const UniValue& params, bool fHelp)
{
    ...
    if (winner) {
        ...
        obj.push_back(Pair("pubkey", EncodeDestination(CTxDestination(winner->pubKeyCollateralAddress.GetID()))));
...
UniValue getmasternodestatus (const UniValue& params, bool fHelp)
{
    if (fHelp || (params.size() != 0))
        throw runtime_error(
            ...
            "  \"addr\": \"xxxx\",        (string) send address for masternode payments\n"
            "  \"status\": \"xxxx\",      (string) Masternode status\n"
...
UniValue getmasternodewinners (const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 3)
        throw runtime_error(
            ...
            "    \"winner\": {\n"
            "      \"address\": \"xxxx\",    (string) send MN Address\n"
