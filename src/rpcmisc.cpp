...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nReturn information about the given send address.\n"
            "\nArguments:\n"
            "1. \"sendaddress\"     (string, required) The send address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"sendaddress\", (string) The send address validated\n"
    ...
    if (isValid) {
        ...
        if (mine != ISMINE_NO) {
            ...
            UniValue detail = boost::apply_visitor(DescribeAddressVisitor(pwalletMain), address);
            ret.pushKVs(detail);
        }
        if (pwalletMain && pwalletMain->mapAddressBook.count(address))
            ret.push_back(Pair("account", pwalletMain->mapAddressBook[address].name));
#endif
    }
    return ret;
}
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2) {
        string msg = "createmultisig nrequired [\"key\",...]\n"
                     ...
                     "2. \"keys\"       (string, required) A json array of keys which are send addresses or hex-encoded public keys\n"
                     "     [\n"
                     "       \"key\"    (string) send address or hex-encoded public key\n"
                     "       ,...\n"
                     "     ]\n"
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"sendaddress\"  (string, required) The send address to use for the signature.\n"
