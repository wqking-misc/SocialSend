...
bool CheckTransaction(const CTransaction& tx, bool /*fNotUsed*/, bool fRejectBadUTXO, CValidationState& state, bool fWitnessEnabled)
{
...
void UpdateUncommittedBlockStructures(CBlock& block, const CBlockIndex* pindexPrev)
{
    ...
    if (commitpos != -1 && GetSporkValue(SPORK_16_SEGWIT_ACTIVATION) < pindexPrev->nTime && block.vtx[1].wit.IsEmpty()) {
...
std::vector<unsigned char> GenerateCoinbaseCommitment(CBlock& block, const CBlockIndex* pindexPrev)
{
    ...
    if (fHaveWitness && GetSporkValue(SPORK_16_SEGWIT_ACTIVATION) < pindexPrev->nTime) {
        if (commitpos == -1) {
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, CBlockIndex* const pindexPrev)
{
    ...
    bool fHaveWitness = false;
    if (GetSporkValue(SPORK_16_SEGWIT_ACTIVATION) < pindexPrev->nTime) {
        int commitpos = GetWitnessCommitmentIndex(block);
        if (commitpos != -1) {
...
bool RewindBlockIndex(const CChainParams& params)
{
    ...
    while (nHeight <= chainActive.Height()) {
        if (GetSporkValue(SPORK_16_SEGWIT_ACTIVATION) < chainActive[nHeight - 1]->nTime && !(chainActive[nHeight]->nStatus & BLOCK_OPT_WITNESS)) {
            break;
        }
        nHeight++;
    }
    ...
    for (BlockMap::iterator it = mapBlockIndex.begin(); it != mapBlockIndex.end(); it++) {
        ...
        if (GetSporkValue(SPORK_16_SEGWIT_ACTIVATION) < pindexIter->nTime && !(pindexIter->nStatus & BLOCK_OPT_WITNESS)) {
            // Reduce validity
...
int ActiveProtocol()
{
    if (IsSporkActive(SPORK_20_NEW_PROTOCOL_ENFORCEMENT_3))
