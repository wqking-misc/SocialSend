...
bool CheckTransaction(const CTransaction& tx, bool fZerocoinActive, bool fRejectBadUTXO, CValidationState& state, bool fWitnessEnabled)
{
    ...
    CAmount nValueOut = 0;
    BOOST_FOREACH (const CTxOut& txout, tx.vout) {
        // Remove this check since Bitcoin Core doesn't have this check too.
        // And the test data in tx_valid for CHECKSEQUENCEVERIFY doesn't have non-empty tx out.
...
void UpdateUncommittedBlockStructures(CBlock& block, const CBlockIndex* pindexPrev)
{
    ...
    if (commitpos != -1 && GetSporkValue(SPORK_16_SEGWIT_ACTIVATION) < pindexPrev->nTime && block.vtx[0].wit.IsEmpty()) {
