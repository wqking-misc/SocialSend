...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake)
{
    ...
    int64_t nBlockMaxCost = DEFAULT_BLOCK_MAX_COST;
    int64_t nBlockMaxSize = DEFAULT_BLOCK_MAX_SIZE;
    bool fCostSet = false;
    ...
    nBlockMaxCost = std::max((int64_t)4000, std::min((int64_t)(MAX_BLOCK_COST-4000), nBlockMaxCost));
    // Limit size to between 1K and MAX_BLOCK_SERIALIZED_SIZE-1K for sanity:
    nBlockMaxSize = std::max((int64_t)1000, std::min((int64_t)(MAX_BLOCK_SERIALIZED_SIZE-1000), nBlockMaxSize));

    // How much of the block should be dedicated to high-priority transactions,
    // included regardless of the fees they pay
    int64_t nBlockPrioritySize = GetArg("-blockprioritysize", DEFAULT_BLOCK_PRIORITY_SIZE);
    nBlockPrioritySize = std::min(nBlockMaxSize, nBlockPrioritySize);

    // Minimum block size you want to create; block will be filled with free transactions
    // until there are no more or the block reaches this size:
    int64_t nBlockMinSize = GetArg("-blockminsize", DEFAULT_BLOCK_MIN_SIZE);
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("SENDMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("SENDMiner : ProcessNewBlock, block not accepted");
...
bool IsMinerValid(CWallet* pwallet) {
    if (Params().NetworkID() == CBaseChainParams::MAIN) {
        if (chainActive.Tip()->nTime < 1510971300) { // Start of PoS phase
            return false;
        }
        if (vNodes.empty() || !masternodeSync.IsSynced()) {
            return false;
        }
    }
...
{
    LogPrintf("SENDMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("send-miner");
    ...
    static int nMintableCheckTime = 5 * 60; // 5 minute check time

    if (fProofOfStake && (GetTime() - nMintableLastCheck > nMintableCheckTime))
    {
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running SENDMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
